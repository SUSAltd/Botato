...
public class FishManager {
	...
	public int catchFish(Fish newFish) {
		int fishPosish = -1; // fish position
		// insert the new fish into the list
		// basically insertion sort kind of maybe
		for (int i = 0; i < fishList.size(); i++) {
			if (newFish.weight() > fishList.get(i).weight()) { // assumes list is already sorted (weight descending)
				fishList.add(i, newFish);
				fishPosish = i;
				break;
			}
		}
		// if the fish still hasn't been added
		if (fishPosish == -1) {
			fishList.add(newFish);
			fishPosish = fishList.size() - 1;
		}
